      ...
      {!isConfigured && (
        <Alert variant="destructive" className="border-2 border-amber-500/50 bg-amber-500/10 text-amber-700 dark:text-amber-300">
          ...
          <AlertDescription className="text-amber-700/80 dark:text-amber-300/80">
            Your Firebase <b>Admin</b> panel is not working because some server settings are missing. Please add the following environment variables to your `.env` file:
            <ul className="list-disc pl-5 mt-2 font-mono text-sm">
              {missingAdminVars.map(v => <li key={v}>{v}</li>)}
            </ul>
            <p className="mt-2">These values can be found in your Firebase project's service account JSON file.</p>
          </AlertDescription>
        </Alert>
      )}
      <Card>
